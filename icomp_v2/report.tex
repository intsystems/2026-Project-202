...
line and the published $WD=0$ control runs both lie on the prediction. (b) Reported
dimension against the dimension of the embedding space. A quantity that is a property of
the data does not depend on the space it is measured in. The Lorenz attractor rises by a
factor of $1.66$ across the range, whereas the weight-norm series rise by $3.73$ and
$4.36$, which is the behaviour of the white-noise reference at $4.40$.}
\label{fig:dimension}
\end{figure}

The second observation concerns identifiability. An intrinsic dimension exists only if the
estimate is insensitive to the dimension of the space in which it is measured.
\Cref{fig:dimension}(b) sweeps that dimension. The Lorenz reference grows by a factor of
$1.66$ from $E_{max} = 5$ to $E_{max} = 30$, while the two weight-norm series grow by
$3.73$ and $4.36$, which is indistinguishable from the factor of $4.40$ obtained for white
noise. On this diagnostic the published observable behaves like noise rather than like an
attractor, so no dimension is identifiable from it.
...
& A straight line reproduces both plateaus; the estimate tracks $E_{max}$ as white noise does
