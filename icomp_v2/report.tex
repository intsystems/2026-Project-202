...
generalise sustain a \emph{higher} velocity than the runs that grok, and the ordering is
consistent across replicate seeds rather than resting on a single draw. The only run far
...
Quantitatively, the runs that never generalise reach $7.26\times10^{-2}$ and
$5.30\times10^{-2}$ over the second half of training against $1.38\times10^{-2}$ to
$1.94\times10^{-2}$ for the three grokking runs, while the run without weight decay falls
to $1.53\times10^{-4}$. Every run with weight decay lies within a factor of five of every
