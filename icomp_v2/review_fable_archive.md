...
### 2.3 MAJOR — the $(m-1)/(m-2)$ explanation does not survive arithmetic

§5.1: *"On the same data LB carries a bias of $+1.02$ against $-0.08$ for MG, which the
$(m-1)/(m-2)$ factor accounts for."*

The source is `dimension_recovery/report_0808.md` §3.1. The LB and MG rows there are
$(1.55, 3.31, 4.48, 5.71, 6.01, 6.07)$ and $(1.22, 2.40, 3.47, 4.13, 4.65, 4.65)$, an observed
ratio averaging **1.32**. Solving $(m-1)/(m-2) = 1.32$ gives $m \approx 5$. But the frozen
configuration uses $m = 20$ (ratio 1.056) and the matrix experiment that Table 3 actually reports
uses $m = 30$ (`exp9_frobenius_k10/best_config.json`, ratio 1.036). At those neighbour counts the
predicted LB–MG gap on estimates averaging 3.4 is $0.19$ and $0.12$ respectively — not the observed
$1.10$.

So one of three things is true and the paper must say which: the quoted figures come from a
configuration with $m \approx 5$ that is documented nowhere; or the agreement is coincidental and
the sentence should be deleted; or there is a second bias source. Show the arithmetic explicitly —
$m$, the mean $\hat d$, the predicted ratio, the observed ratio — in one line of Appendix C. As
written it is an unverifiable claim of theoretical agreement, and it is exactly the kind of claim a
referee checks.

Compounding this: those LB/MG figures come from a **different experiment** (ranks 1–6, a different
configuration) than the recovery numbers quoted two sentences earlier in the same paragraph (ranks
1–10, `exp9`). §5.1 currently reads as one experiment. Say when the data changes.
...
### 3.1 CRITICAL — Appendix L's statement about the Theiler cap is exactly inverted

Appendix L: *"the cap binds on the numerator and not on the denominator whenever
$t_{\mathrm{acf}} \ge 150$, [...] The two arms of the ratio are then not treated identically."*

The implementation is $W_T = \min(\max((E-1)\tau,\, t_{\mathrm{acf}}),\, 150)$
(`mg.py:156`, `THEILER_CAP = 150` at `mg.py:42`; rule at `edm/dimension.py:84-85`). Therefore, at
the eight-direction configuration:

- numerator ($E = 40$, $\tau = 4$, span 156): $W_T \equiv 150$, capped **unconditionally**;
- denominator ($E = 20$, $\tau = 4$, span 76): $W_T = 76$ if $t_{\mathrm{acf}} \le 76$;
  $= t_{\mathrm{acf}}$ if $76 < t_{\mathrm{acf}} < 150$; $= 150$ if $t_{\mathrm{acf}} \ge 150$.

The two arms are treated **identically if and only if $t_{\mathrm{acf}} \ge 150$**, and differ when
$t_{\mathrm{acf}} < 150$. The appendix asserts the opposite. Consequences worth stating, because
they change the conclusion of that appendix rather than merely correcting a sentence:

- the five parameter-norm records the appendix flags as contaminated ($t_{\mathrm{acf}} = 161$,
  $335$, $434$, $857$, $858$) are exactly the **clean** ones;
- the calibration systems have $t_{\mathrm{acf}} = 1$–$3$, so on the data that produced the
  reference band $1.00$–$1.09$, the two arms differed in **every** cell (76 against 150). The
  diagnostic's calibration is the contaminated case, not the application.

Appendix L's closing recommendation ("lifting the cap is the first thing to change in any
follow-up") survives and is strengthened; the diagnosis above it does not.

### 3.2 MAJOR — "the eight-direction configuration is uncapped" is false on the grokking logs

Appendix B: *"The eight-direction configuration is uncapped."* Appendix L: *"the cap never binds."*
The cap is applied unconditionally at `mg.py:156`. On the transformer logs
$t_{\mathrm{acf}} = 161$–$858$ (the paper quotes this range itself, in §6.2) and on the perceptron
training-loss logs $t_{\mathrm{acf}} \approx 661$–$695$, so the exclusion actually used on five of
seven transformer records and on all ten perceptron records was $150$, not the value the stated rule
requires. The claim is true only of the calibration data. The bias direction is the one the paper's
own Appendix L measures: a short exclusion inflates the estimate.

### 3.3 MAJOR — the "frozen configuration" was not the configuration applied to the grokking logs

§7.3 opens: *"Applying the frozen configuration of §5.3 to the logs of both settings [...]"*, and
Table 2 gives window/stride $8\,000/2\,000$. The code overrides both:
`window = min(cfg.window, max(2000, len(x)//3))`, `stride = 1000`
(`e5_real_logs.py:66-67`, `dimension_probe.py:72-73`). The transformer logs have 12 000 samples, so
the window used was **4 000 with stride 1 000** (nine windows, matching `n=9` in the summary CSV);
the perceptron logs have 10 000 samples, so **3 333 with stride 1 000**.

The code comment gives a sound reason (an 8 000 window admits only three placements in a 12 000
sample record). The paper does not mention the deviation at all, while making "frozen and applied
unchanged" a load-bearing claim of its protocol. Disclose it, and state whether the eight-direction
reference band for $\rho_{\mathrm{ident}}$ was recomputed at the window actually used.

Related, and smaller: the eight-direction *evaluation* also did not use the frozen stride —
`e2_rank_sweep.py:113` sets `stride = max(500, (n - window)//6) = 3000` against a frozen 2 000.

### 3.4 MAJOR — two rows marked "complete" in Table 3 were produced by code that breaches the protocol they claim to satisfy

The `protocol` column of Table 3 marks the oscillating-matrix row and the image-data function-subspace
row as **complete**, i.e. satisfying every requirement of §4.

*Oscillating matrix* (`exp9_frobenius_k10.py`, via `estimators.py:39-42`):

- runs with `theiler_window = 0` — **no Theiler exclusion at all**, contradicting Algorithm 1;
- runs with `clamp_to_max_E = True`, contradicting "no clamping of the returned value to $E$ is
  applied" (Appendix A) and the reasoning in §6.1 that rests on it;
- its selection grid sweeps `cycles`, a **system** parameter, breaching requirement 2 ("a grid of
  estimator parameters only") — the very requirement whose breach disqualifies the regression
  systems two rows below;
- computes none of the three companion statistics, breaching requirement 5;
- and uses $E = 25$, $m = 30$, window 4 000 — not the frozen eight-direction configuration.

*Image data, function subspace* (`exp15_real_digits_functional_subspace_v3.py`): clamping on;
Theiler set to the embedding span only; no degeneracy flag; **one window per run**, contradicting
Table 3's own caption ("each side of the error being the median over that run's sliding windows");
a selection grid mixing system parameters (`eta`, `cycles`) with estimator parameters; selection on
seed 0 at **all** ranks 1–8 with seeds 1–2 held out at the **same** ranks, breaching requirement 3;
**no zero-learning-rate control anywhere in v3**, so requirement 4 is untested. Its winning observer
is `gradient_projection`, a residual-dependent observer — the same class that requirement 4
disqualifies elsewhere in the paper.

This is the most serious presentational problem in the paper after §1.3, because the protocol is the
contribution. Either re-run these two systems under the canonical estimator and the frozen
configuration, or re-label the column honestly. A protocol whose compliance column is wrong is worse
than no column.

### 3.5 MAJOR — Table 3's caption misdescribes the matrix row's two numbers

Caption: *"the two oscillating-matrix figures are its two observers."* They are two **held-out
seeds** (seed 1: MAE $0.302$; seed 2: MAE $0.693$) under a single observer, the Frobenius norm
(`results/exp9_frobenius_k10/report_exp9.md`). `exp9` has one observer.

### 3.6 MAJOR — "accurate to within half a component up to ten directions" is not what the data shows

§5.1. The held-out medians in `report_exp9.md` are $+0.104, +0.117, +0.208, +0.556, +0.211, -0.250,
+0.198, -0.307, -0.815, -1.509$ for $k = 1 \dots 10$. The error exceeds half a component at $k = 4$,
$9$ and $10$, reaching $1.5$ at $k = 10$. As a mean it is $0.30$ for one held-out seed and $0.69$
for the other. Neither reading supports "within half a component up to ten directions". Say
"a mean absolute error of $0.30$ and $0.69$ on two held-out seeds, with per-rank error rising to
$1.5$ components at ten" — which is also the evidence for the ceiling story of §1.5.
...
- **Table 1, "27–29"** for the transient estimate. Under the paper's own aggregation (median over
  the ten usable observers) the per-rank values are $28.88$–$29.25$, so the cell should read
  $\approx 29$. The $27$ is reachable only through the two observers the paper excludes from every
  aggregate, and the upper end understates `fn_proj1`'s $29.75$.
...
### 8.1 CRITICAL — the transformer setting is Nanda et al., not Omnigrok

§2, §7 and Table 16 attribute the regularised stochastic setting to `liu2022omnigrok`: one-layer
transformer, $d_{\mathrm{model}} = 128$, four heads, $p = 113$, training fraction $0.30$, AdamW at
$10^{-3}$ with $\beta = (0.9, 0.98)$, weight decay $1.0$.

That is, to the digit, the configuration published in **Nanda et al. 2023** (already in your
bibliography, cited only for the cleanup phase): one-layer ReLU transformer with no LayerNorm,
untied embed/unembed, $d_{\mathrm{model}} = 128$, four heads of dimension 32, $d_{\mathrm{mlp}} =
512$, $p = 113$, 30 % training fraction, AdamW, lr $10^{-3}$, $\beta = (0.9, 0.98)$,
$\varepsilon = 10^{-8}$, weight decay $1$, cross-entropy. Omnigrok says so itself: *"We use the
setup of [Nanda et al.] and train a 1-layer transformer on modular addition ($p = 113$)."* Omnigrok's
contribution is the LU mechanism and the weight-norm analysis, not the recipe.

Your own code confirms it. `code/grokking_train/grok/models.py` defines `OmnigrokTransformer` with
`d_model=128, d_mlp=512, d_head=32, num_heads=4, n_ctx=3`, LayerNorm-free, with parameters named
`W_E, W_U, W_pos, W_K, W_Q, W_V, W_O, W_in, W_out` — verbatim Nanda's implementation, which
Omnigrok's own repository copies. **The class name is the source of the error**, so rename it or the
misattribution will propagate into the next paper too.

Fix: "the regularised stochastic setting of \citet{nanda2023progress}, as used by
\citet{liu2022omnigrok}", at §2, §7 and Table 16's caption.

### 8.2 MAJOR — three further errors in the same experimental description

- **Omnigrok contains no $S_5$ experiment.** Its tasks are teacher–student regression, MNIST, IMDB,
  QM9 and modular addition. $S_5$ composition is Power et al. 2022, which you already cite.
- **"Mini-batches" belongs to neither cited source.** Nanda et al. train **full-batch**; Omnigrok
  states no batch size; Power et al. use 512. Your batch size of 256, and the weight decay of $0.2$
  on $S_5$, are your own choices. This one is not cosmetic: the paper's entire §3.3 and §7.3
  argument rests on those runs being *stochastic*, which is precisely the respect in which they
  depart from the published configuration. Say so explicitly — it converts a hidden deviation into a
  deliberate design choice.
- **Doshi et al. 2024 is not unregularised.** §2 and §7 describe "the unregularised full-batch one of
  \citet{gromov2023grokking} and \citet{doshi2024grokking}". Doshi's Appendix C uses Adam,
  lr $5\times10^{-3}$, **weight decay $5.0$**. Your own `code/gromov_polynomials/README.md` says it:
  *"this paper, unlike its predecessor, does regularise, and heavily."* You take the optimisation
  setting from Gromov and the tasks and representability criterion from Doshi. Write it that way.

### 8.3 MAJOR — Nanda and Varma are mischaracterised, and the mischaracterisation is load-bearing

§7.1: *"Both are statements about the weights at the end of training; this is one about the
trajectory while it moves, and they agree."*

Neither is an end-of-training statement. Nanda et al.'s entire contribution is *progress measures
computed throughout training* — restricted loss, excluded loss, Gini of the Fourier norms — used to
partition training into memorisation, circuit formation and cleanup at specific epoch ranges. Varma
et al. argue dynamically too, and predict ungrokking and semi-grokking as trajectory phenomena.

The sentence is the pivot on which §7.1 claims novelty, so it needs a replacement that survives
contact with the sources. Something like: *both describe what the weights encode as training
proceeds; ours describes the geometry of the trajectory itself.* The "and they agree" conclusion is
unaffected and remains a good observation.

### 8.4 MAJOR — Theiler 1986 says the opposite of what the paper attributes to it

§2: *"temporally adjacent neighbours **inflate** the estimate on short records
\citep{theiler1986spurious}."*

Theiler 1986 shows that temporally correlated pairs produce a spuriously **low** apparent dimension:
at small radii the trajectory looks like a smooth one-dimensional curve, biasing $D_2$ toward 1.
That is why he introduced the exclusion window. Standard restatements (Kantz & Schreiber §6.4–6.5;
the TISEAN documentation) describe "the strong bias to $D_2 = 1$".

Note that your own Appendix L measures *inflation* for the MLE estimator ($2.78$ at $W_T = 150$
against $3.13$ at $W_T = 624$). That is your empirical finding and it is interesting precisely
because it runs the other way; report it as yours, and drop "in the direction that a shortage of
exclusion predicts", which currently cites Theiler for the opposite of what he found. The Theiler
citation in §3.2, attached to the exclusion itself, is correct.

### 8.5 MAJOR — the Stark critique is partly unfair, and contradicts the paper's own §2

§3.3: *"the embedding theorems for forced systems assume a forcing with its own dynamics, whereas
mini-batch noise is state-dependent and drawn from a finite dataset. What they embed is in any case
the state with its noise history, a skew product rather than an $r$-dimensional attractor."*

Point by point against Stark et al. 2003, Theorems 2.3–2.6:

- *"a forcing with its own dynamics"* — **fair**. Both papers need an autonomous exogenous driver.
- *"state-dependent"* — **not a valid objection.** Stark's model is $x_{i+1} = f(x_i, y_i)$: the
  update may depend on the state arbitrarily; only the driving *sequence* must be exogenous, and in
  SGD the batch-index sequence is drawn independently of the weights. A state-dependent gradient-noise
  covariance is exactly what $f(x,y)$ covers.
- *"drawn from a finite dataset"* — **valid, but anticipated.** It does break Theorem 2.3, whose
  hypothesis is absolute continuity of $\mu_\Omega$ on $(d-1)$-cylinders, and a uniform measure over
  finitely many batches is atomic. But Theorem 2.4 covers iterated function systems — finitely many
  maps chosen at random — and gives an embedding *for all* $\omega$. Mini-batch SGD on a finite
  dataset is such a system.
- *"what they embed is the state with its noise history"* — **misstates the theorem.**
  $\Phi_{f,\phi,\omega}$ embeds the state manifold $M$ alone, fibre-wise, for almost every $\omega$,
  at $d \ge 2m+1$ with $m = \dim M$ and the forcing dimension excluded.

The defensible version, which I would use: the bundle theorems give a *fibre-wise* embedding for a
fixed driving realisation; a single observed orbit traverses different fibres, so the reconstructed
cloud from one run is not confined to the image of any $m$-dimensional set, and there is no
invariant measure on a low-dimensional set for the neighbour statistic to sample. Cite Theorem 2.3's
absolute-continuity hypothesis explicitly and acknowledge Theorem 2.4.

**Internal contradiction.** §2 says mini-batch training near a minimum is "a stochastic process
whose delay map embeds the state with its innovations \citep{stark2003delay}, which is the third
regime"; §3.3 says the third regime is exactly where those theorems fail. The same source is cited
for opposite conclusions about 120 lines apart. One of them has to go.

Relatedly, Table 1's third row says "no compact invariant set". In the random-dynamical-systems
framing Stark works in, $M$ is compact and a random pullback attractor exists. Write "no
low-dimensional deterministic invariant set".
...
### 8.8 Smaller mischaracterisations

- `pope2021intrinsic` is about **image/pixel space**, not activation space; §2 groups it with
  Ansuini et al. under "a cloud of points in activation space". Write "in data or activation space".
- `gurari2018gradient`: the top Hessian eigenvectors are obtained by Hessian-vector products and
  Lanczos, not "measured from stored gradients". The author spelling in the bib is correct.
- `kumar2024grokking` is a lazy-to-rich dynamical-regime theory, not a progress measure defined in
  weight or activation space.
- Table 16: "reproducing the configuration **the published** $S_5$ logs were generated with" reads,
  in context, as Liu et al.'s. Your own `grokking_train/README.md` calls this "the double-step bug"
  in your earlier notebooks. Rephrase to "our own previously released $S_5$ logs, which contain a
  duplicated `optimizer.step()`" — a reader who takes the current sentence at face value will look
  for a double step in a published paper and not find it.

### 8.9 Bibliography defects

| key | problem | fix |
| --- | --- | --- |
| `liu2022omnigrok` | the OpenReview id in the URL is wrong; the link is dead. Key says 2022, `year` says 2023 | id `zDiHoIWa0q1` |
| `cao1997practical` | author is **Liangyue** Cao, not "Cao, Liang" | correct the given name |
| `sauer1991embedology` | issue is 3–4, not 3 | `number={3-4}` |
| `mackay2005comments` | typed `@techreport` with an institution and **no URL**; it is an unpublished web note and a reader cannot find it | add the URL and `note={Unpublished note}`, or switch to `@misc` |
| `mandt2017stochastic` | DBLP cumulative pagination; the article is JMLR 18(134):1–35 | prefer the canonical form |
| `mccandlish2018empirical` | `Team, OpenAI Dota` renders as "Team, O. D." | brace the corporate author |
| `jiang2019fantastic`, `keskar2016large`, `sagun2018empirical` | key year differs from the `year` field | cosmetic; all three are currently uncited |
| ten dynamics-and-sketching entries | no DOIs while most others carry them | consistency only |

**Twenty-three of sixty entries are never cited.** The CCM and EDM cluster
(`leng2020partial`, `sekaran2024self`, `lin2024novel`, `brouwer2021latent`, `usmanova2019models`,
`ye2015*`, `rulkov1995generalized`, `sugihara1994nonlinear`) is a leftover from the earlier
CCM-focused draft and can be deleted outright. Several of the rest should be cited instead of
removed — see 8.7. No cited key is missing from the bibliography.
...
Fifteen items, in the order I would do them. The first four are correctness and should be done
before anything else; the rest are the paper.
...
2. **Two "protocol complete" rows were produced by code that breaches the protocol.** The compliance
   column is the contribution, and it is wrong. *(3.4)*
3. **Appendix L's Theiler statement is exactly inverted** relative to the code, and its conclusion
   changes with it. *(3.1)*
4. **The frozen configuration was not the one applied to the grokking logs** — window and stride were
   overridden, undisclosed. *(3.3)*
5. **Reconcile the overclaims**, all of which are cheap to fix and all of which a referee can check:
   "within half a component up to ten", "on six systems", "orders it to twenty", "at the onset of
   generalisation", "a factor of four across seven runs", "$28.6$ to about $6$", "falls by more than
   half", "$r = 1, \dots, 8$". *(3.6, 3.12, 3.13, 1.9, 4.2, 4.5)*
6. **Figure 1(b) is computed on 54 numbers from one seed** and its caption says otherwise. Recompute
   $\rho_{\mathrm{ident}}$ over the full sweep — the diagnostic's calibration is the paper's thinnest
   evidence. *(5.1)*
7. **Correct the Nanda/Omnigrok attribution**, the $S_5$ attribution, the mini-batch deviation and
   the "unregularised Doshi" description. Four errors in one experimental description. *(8.1, 8.2)*
8. **Reorder §7 so the instrument is present at the headline result**, or add a roadmap sentence.
   *(1.3)*
9. **Move Figure 3 and Table 3 into the body**, paid for from §2 and the repetitions. *(6.2, 6.3)*
10. **Turn the saturation ceiling into a result** — $E_{\max}/2$ predicts every saturation point in
    the paper, and $E_{\max} = 40$ cannot recover twenty components by construction. Test it against
    the Eckmann–Ruelle record-length bound. *(1.5, 2.2, 8.7)*
11. **Fix the estimand definition** — a finite orbit closure is a curve; define it at a resolution
    $\varepsilon$ and §6.4 becomes a confirmation rather than a caveat. *(2.1)*
12. **Run the sliding-window estimate on the grokking logs**, or withdraw §7.3's constructive claim.
    The 281 windows are already computed. *(1.6)*
13. **Correct the Nanda and Varma characterisation** in §7.1 — they are dynamical accounts, not
    end-of-training ones, and that sentence is the pivot of the paragraph. *(8.3)*
14. **Confront whether the admissible regime is ever realised in undriven training.** The
    edge-of-stability run is cheap and would give the instrument its first real domain. *(1.10)*
15. **Regenerate Table 7's twenty-direction column** and print $\PR_{\mathrm{delay}}$ and the
