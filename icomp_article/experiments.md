...
Two of the eight section 7 checks came out **against** the audit's expectation. They are
reported first, as corrections, because they change what can be said.
...
| | Does the drop conjunct discriminate? | **Recall yes** (10/11 at window 600), **specificity no**: `lowdata15` and `lowdata20` fire at almost every setting. |
...
```
run           gen    gap    d30/s1    d30/s3    d30/s5    d40/s5
grok            Y  12070     +9410     +9210     +5910     +5810
grok_s1         Y   1750         -         -         -         -
grok_s2         Y   1740      +650      +450         -         -
lowdata15       N     --       FIRE      FIRE         -         -
lowdata20       N     --       FIRE      FIRE         -         -
wd0             N     --          -         -         -         -
...
The 0/3 column is one step away from the 2/3 column. Both controls fire at sustain 3 and
neither fires at sustain 5, which places the threshold between two controls rather than
between two families. And the sweep has 4 drops × 3 sustains × 3 windows = 36 cells; if
each control fired independently with probability one half, about 4 cells in 36 would
show 0/3 by chance. One did.
...
   reproduces across the conf set. But `lowdata15` and `lowdata20` do the same thing, so
   recall is not the problem and never was.
4. **The published 3000-step window cannot be repaired by tuning.** It is longer than the
   plateau in 5 of 11 generalising runs.

**What the missing conjunct has to do.** It must separate `grok` from `lowdata` — runs
...
straightness. Nothing measured in this repo so far separates them. That is the honest
statement of where the project is, and it is an argument for the Tier 6 ensemble redesign
rather than for another statistic computed from the same logs.
...
| §12 | add: "the estimator cannot resolve a dimension change" is **not** permitted — §1.2 above |

## 14. What is now worth doing, in order

1. **Use the 72-setting sign flip in the paper.** It needs no theory, no new runs, and it
   is the cleanest falsification available. One table.
2. **State the precondition result** (§3) beside it: below ~5 cycles per window every
   system returns the tangent constant, and `weight_norm` windows contain none. This
   explains *why* the sign flips, and it is a positive contribution rather than a
   retraction.
3. **Stop proposing conjuncts computed from the same six logs.** §12 point 4 is the
   binding constraint: nothing in the existing data separates `grok` from `lowdata`.
4. **Tier 6 ensemble sweep**, unchanged from `report_0708.md` §7. It is now the only
   remaining route to a claim about dimension, and §5 above sharpens why: on an ensemble
   at fixed t the sample is i.i.d. by construction, so the roughness confound that
   explains 87 % of the rank variance here does not exist.
5. **[open]** E0.4, the eigenvalue multiplicity check, is still unmeasured and still
