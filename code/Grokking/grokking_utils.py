...
def get_s5_composition_data(fraction=0.5, seed=42, device='cpu'):
    torch.manual_seed(seed)
    
    perms = list(itertools.permutations([0, 1, 2, 3, 4]))
    num_elements = len(perms)
    
    perm2id = {p: i for i, p in enumerate(perms)}
    id2perm = {i: p for i, p in enumerate(perms)}
    
    table = torch.zeros((num_elements, num_elements), dtype=torch.int64)
    for i in range(num_elements):
        for j in range(num_elements):
            a = id2perm[i]
            b = id2perm[j]
            c = tuple(a[b[k]] for k in range(5))
            table[i, j] = perm2id[c]
            
    equals_token = num_elements
    x_idx, y_idx = torch.meshgrid(torch.arange(num_elements), torch.arange(num_elements), indexing='ij')
    x_idx = x_idx.flatten()
    y_idx = y_idx.flatten()
    ...
    answers = table[x_idx, y_idx].to(device)
    ...
    return X_train, Y_train, X_test, Y_test, num_total, num_elements

def plot_grokking_replication(df_logs, title="1L Transformer", save_path=None):
    fig, ax1 = plt.subplots(figsize=(9, 5.5))
