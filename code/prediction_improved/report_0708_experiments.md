...
lowdata15            0.00023      0.03x      ~0       rho -0.07          CENSORED (groks at 110940)
...
run, which was read here as evidence that it is not approaching a transition. **Superseded 2026-08-08.** The 120 000-step reruns in `../dimension_recovery/exp8_extended.py` show that `lowdata15` generalises at step 110 940 and `lowdata20` at 39 600. Neither is a negative, and the seed that generalises was 0.55x chance with a falling trend at step 20 000 — the exact profile used below to argue the opposite. **No control in this repository is established as a negative, so no specificity or false-alarm figure anywhere in these documents is valid.**
That reading was wrong, and the paragraph is left standing so the error stays auditable. `wd0` does not change by a single validation example in 19 000 steps.
...
lowdata15       N     --       FIRE      FIRE         -         -   CENSORED, groks at 110940
