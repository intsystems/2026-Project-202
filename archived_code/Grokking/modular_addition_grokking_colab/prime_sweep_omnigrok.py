...
def make_tensorboard_writer(run_dir: Path, cfg: Config, p: int, seed: int):
    ...
    # Training is not resumed from a checkpoint, so stale event files would
    # produce overlapping/duplicated steps. Every fresh run gets a clean log.
    if tb_dir.exists():
...
def train_one(cfg: Config, p: int, seed: int):
    ...
    if done.exists() and cfg.skip_completed and not cfg.force_restart:
        print("Skipping completed", run_dir); return json.loads(done.read_text())
    ...
    writer = make_tensorboard_writer(run_dir, run_cfg, p, seed)
    text_log = TextRunLogger(run_dir/run_cfg.text_log_filename, run_cfg.text_log_enabled)
    text_log.log(
    ...
    logs=[]; memo_step=gen_step=first_generalization_step=None
    ...
    # Faithful equivalent of DataLoader(..., shuffle=True) in the successful
    # project notebook: one random permutation per epoch, including a smaller
    # final batch instead of discarding it.
    train_loader = DataLoader(
        TensorDataset(xtr, ytr), batch_size=effective_batch_size,
        shuffle=True, drop_last=False,
        num_workers=0,
    )
    train_iterator = iter(train_loader)
    start_time=time.time(); examples_seen=0
    for step in tqdm(range(1,run_cfg.max_steps+1), desc=f"Omnigrok mod {p}, seed {seed}"):
        ...
        row=dict(step=step,examples_seen=examples_seen,epoch_equivalent=examples_seen/len(ytr),
                 ...
                 parameter_participation_ratio=weight_pr,elapsed_seconds=time.time()-start_time)
        for r in range(run_cfg.projection_count):
        ...
        if step%run_cfg.checkpoint_every==0:
            torch.save(dict(step=step,model=model.state_dict(),optimizer=opt.state_dict()),run_dir/"checkpoint.pt")
    ...
    summary=dict(completed=True,p=p,seed=seed,**data_meta,
                 ...
                 gap_steps=(gen_step-memo_step if gen_step is not None else None),final_step=logs[-1]["step"],
                 final_train_acc=logs[-1]["train_acc"],final_val_acc=logs[-1]["val_acc"],
    ...
    torch.save(dict(step=logs[-1]["step"],model=model.state_dict(),optimizer=opt.state_dict()),
